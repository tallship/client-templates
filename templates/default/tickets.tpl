				...
				{/foreach}
				  ...
				  <div class="deptsdesc" >
                    {foreach from=$depts item=dept name=fff}
                        <div class="dept_{$dept.id}" {if $submit.dept_id && $submit.dept_id==$dept.id} style="display:block" {elseif $smarty.foreach.fff.first}  style="display:block"  {/if}>{$dept.description}</div>
                    {/foreach}
                    {foreach from=$depts item=dept name=fff}
